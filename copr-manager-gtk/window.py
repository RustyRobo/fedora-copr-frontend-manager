...
class MainWindow(Adw.ApplicationWindow):
    ...
    def on_search_changed(self, entry):
        ...
        self._search_timeout_id = GLib.timeout_add(500, self._do_search, query)
    ...
    def _generic_repo_action(self, repo, action):
        ...
        # Validate owner/project before passing to DNFManager.
        try:
            owner, project = sanitize_full_name(full_name)
        except ValueError as e:
            dialog = Adw.MessageDialog(
                transient_for=self,
                heading="Invalid Repository",
                body=str(e),
            )
            dialog.add_response("ok", "OK")
            dialog.present()
...
class RepoDetailsWindow(Adw.Window):
    def __init__(self, repo, parent_window):
        ...
        # Homepage (escape URL before displaying)
        homepage = repo.get('homepage')
        if homepage:
            row_home = Adw.ActionRow()
            row_home.set_title("Homepage")
            row_home.set_subtitle(GLib.markup_escape_text(str(homepage)))
        ...
        if contact:
            ...
            row_contact.set_subtitle(GLib.markup_escape_text(str(contact)))
    ...
    def on_enable_clicked(self, btn):
        # Single definition — calls _generic_repo_action which validates inputs.
        if hasattr(self.parent_window, '_generic_repo_action'):
